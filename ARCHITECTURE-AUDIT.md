...
2. **Phase 2c (stores and lifecycle):** carve per-domain stores out of `state.js` (`load()` memoized, `get()`, `subscribe()`, domain writes owning optimistic/pessimistic policy) one view at a time, starting with Tasks and Invoices; give the router `mount(el, params)` / `unmount()` per view with lazy `import()` so a deep link to a board loads the profile and the board and nothing else; move the pagehide/visibility flushes into `unmount`. With modules and one state object in place this is incremental per view, not a rewrite, and each step keeps the harness green.
