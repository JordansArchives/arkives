...
**2026-09-04, Phase 2a: delegation and an enforced CSP.** Commit `bb067a8`. All 181 inline handlers (app.js 61, invoices.js 48, outreach.js 40, boards.js 11, toolkit-views.js 9, index.html 10, plus the inline theme script) are gone. One dispatcher at the top of `app.js` listens for click, input, change, keydown and submit on `document` and calls handlers registered with `act({ ... })` in the file that defines them; markup carries `data-action` (or `data-input` / `data-change` / `data-keydown` / `data-submit`) and HTML-escaped JSON args built by `_args(...)`, with `"$event"`, `"$el"`, `"$value"` and `"$checked"` resolved at dispatch. Semantics match the inline form exactly: a handler gets only the args its markup lists, events walk up through nested actionable elements the way inline handlers bubbled, and `data-stop` (or a handler calling `stopPropagation`) ends the walk and hides the event from later document listeners. The two backdrop-close handlers that compared against `event.currentTarget` now take the element. The Boards toolbar, which already used `data-action` for its own listener, moved to `data-cmd`. `public/_headers` now sends an enforced `Content-Security-Policy` with no `'unsafe-inline'` for scripts, which closes the stored-XSS class from Section 4 for good: injected markup cannot run script even if an escape is missed. The test server sends the production headers, the smoke stubs no longer use `eval`, and `tests/checks.mjs` gained six static guards (no `on*=` attribute or inline `<script>` in `public/`, every action name in the source is registered, constant args parse, the CSP is enforced with no unsafe-inline or unsafe-eval) plus 30 dispatcher and click-through checks that drive the real Settings, Calendar, Tasks, Invoices, Outreach, Boards and auth UI through the dispatcher. jsPDF verified to render under the policy. Harness: lint clean, 94/94 checks, smoke 0 errors. A planted `onclick` fails checks with the file and line.

**2026-09-04, Phase 2b: native ES modules, no bundler.** Commit `8fa0055`. Jordan chose native modules over Vite (zero build step, no deploy coordination). The five classic scripts (9,411 lines, one shared scope with 489 globals) became 25 modules under `public/src/`: `main.js`, `state.js`, `router.js`, `lib/` (actions, sb, toast, esc, format, icons, storage, share) and `views/` (auth, dashboard, revenue, mediakit, analytics, inbox, calendar, tasks, settings, scripts, invoices, outreach, boards, contracts). The conversion was AST-driven (espree + eslint-scope, the parser ESLint already ships): every one of the 100 module-level `let`/`var` bindings became a property of one exported `state` object and every reference was rewritten by range, so cross-module assignment (which ES module bindings forbid) is a non-issue; all 34 Supabase functions moved to `lib/sb.js` and are called as `db.sbX(...)` through one exported object, which is the seam tests use to swap I/O; imports and exports were computed from the reference graph (function-only cycles between views and the router are fine in ES modules; no constant initializer crosses a module boundary, so there is no TDZ exposure); the 14 top-level side effects (dispatcher registration, Supabase client, auth listener, hashchange, theme, mobile nav, visibility/pagehide flushes, popover closer) moved into per-module `__init()` hooks that `main.js` runs in a fixed order before `boot()`, so evaluation order no longer depends on the import graph. `main.js` publishes `window.__arkives` (state, db, every export), and the harness was rewritten mechanically onto it (255 references), so the safety net survived the move: lint clean with `no-undef` now meaning what it says (no generated global list) plus `no-unused-vars`, 102/102 checks, smoke 0 errors under the enforced CSP. New guards: every module is preloaded from `index.html` and every preload exists; `index.html` loads only `src/main.js`; the client exists after `sb.__init`; boot completes; module scope leaks nothing onto `window`; the hashchange route works; the action registry logs no duplicate or missing handler. `tools/app-globals` is deleted. Three dead locals the stricter lint found were removed.

**2026-09-04, Phase 2c: stores and lifecycle.** Commit `0d581f3`. A store factory (`stores/_store.js`: `load()` memoized per session and a no-op without a session, `reload()`, `reset()`, `subscribe()`, `notify()`, the `keys` it owns) and eleven stores. The 260-line boot loader became one loader per domain in `lib/sb.js` (`sbFetchProfile`, `sbFetchDeals`, ... `sbFetchInvoices`), each returning false on a hard failure so the store stays unloaded and the next navigate retries, with the old tolerance for optional tables kept (missing-table flags, empty data on query errors). `stores/index.js` declares `VIEW_STORES`: which stores each view needs. Boot now loads the session, the profile, and only the first route's stores before the first paint; every later navigate loads what its view needs behind a skeleton the first time and is synchronous after that; a deep link to a board or script loads the profile and nothing else. Tasks, invoices and clients own their writes (`tasks.add/update/toggleComplete/toggleStar/remove(undo)/flushDeletes/removeMany`, `invoices.save/setStatus/recordPayment/remove`, `clients.add/update/remove`); the views read inputs, compute totals, and let the router re-render them through their store subscription. The router has a view registry with `unmount` hooks: leaving Tasks commits pending deletes, leaving Invoices closes the body-mounted row sheet (which used to outlive the view), leaving a script or board editor flushes saves and leaves the Realtime channel, and moving from one script to another flushes the first because editors are keyed by their full hash. Logout resets every store. Harness: lint clean, 122/122 checks, smoke 0 errors; new checks cover the store contract, the guard that every view's `VIEW_STORES` entry covers the store-owned keys it reads (source-scanned), unmount flushes for Tasks, Invoices and the script editor, subscription re-render and unsubscribe, and every loader driven against a fake Supabase client with canned rows (profile with platforms and rate card, missing tasks table, invoices with clients, dashboard set, outreach, audience, inbox, campaigns), plus the skeleton-then-render path of a signed-in first visit and a throwing loader leaving its store unloaded.

**2026-09-10, Ideas.** A new daily tool, not an audit phase, but built to the Phase 2d contract from the start so it adds nothing to the finish plan: `stores/ideas.js` owns `state.IDEAS` and every write (add and edit wait for the row, archive and restore are optimistic with revert, delete waits five seconds for Undo and commits on unmount, tab hide or unload); `views/ideas.js` renders, reads inputs, and never touches `db`. Migration `022_ideas.sql` adds the table with the 010 RLS pattern (idempotent; the view explains itself until it is applied). Sidebar entry under Tools next to Scripts; phone reaches it through More, the tab bar stays at four. Harness: lint clean, 138/138 checks (14 new: escaped render order, optimistic archive and its revert, undo-able delete, unmount flush, body-mounted modal, composer add-and-stay-open, textarea Enter vs Cmd/Ctrl+Enter, missing-table state, loader against the fake client), smoke 0 errors across 14 views. Found and fixed on the way: the Tasks edit modal stayed open after Save (a re-render never hid it); both modals now close explicitly and a check covers it.

**2026-09-10, later: Ideas became a notepad.** Jordan's call after using it: no composer, no button in the header, the line should be ready the moment the view opens. The view is now a ruled sheet: `.ideas-sheet` draws a rule every `--rule` (32px; 36px on phones where inputs must be 16px) with a repeating gradient, and every line of text on it, capture line, title, notes, inline inputs, has `line-height: var(--rule)` and no vertical padding, so the text sits on the lines and nothing drifts. The capture line is a bare input with an Add button at the end; Enter adds and keeps the caret (it autofocuses on pointer devices only, so a phone does not open its keyboard over the list). Editing is in place: tap a line and it becomes an input with an auto-growing notes textarea under it; Enter or Cmd/Ctrl+Enter commits, Escape cancels, focus leaving the entry commits (a document `focusout` listener in the view's `__init`, checked a tick later so moving between title and notes does not count), leaving the view commits synchronously. The store did not change. A proportion pass followed: one 20px inset both sides on an 8px grid, the right column (`.pad-end`) is one cell as wide as the actions and flush with the Add button, the date shows at rest and the actions replace it in place on hover, focus or edit. Harness: 151 checks (the notepad block replaced the composer/modal block and grew by thirteen: the right-column alignment, the ink focus cue, a wrapping title while editing, capture-line focus and Enter, empty Enter, inline edit open/commit/cancel/blur/refuse-empty/save-on-leave, and a mechanical guard that every line on the sheet is a whole number of rules tall in view and edit mode), smoke clean.

**2026-09-11, Boards: edit on double-click, text alignment, pen stabilization.** Three fixes from use, no audit phase. (1) Double-clicking an item spawned a sticky on top of it instead of editing: pointer capture on the viewport makes Chrome retarget `click`/`dblclick` at the viewport, so `e.target.closest('.bd-item')` was always null. The handler now recovers the real target from the pointerdown (recorded before capture, `state._bdDownTarget`) or a hit test (`_bdHitTarget`), and `_bdEditItem` edits whatever text the item carries: stickies and text blocks their body, images and videos their caption. Phones get the same through a touch double-tap detector in `endGesture` (two still taps on one item inside 350ms/24px), since mobile browsers do not reliably fire `dblclick`. (2) The format bar gained left/center/right alignment as a whole-item property (`content.align`, allowlisted on render by `_bdAlignOf`, class `bd-ta-*`, undo-able; the 020 validator ignores unknown keys so shared-edit RPCs accept it). (3) The pen strip gained a four-step stabilization control (`BD_PEN_STAB`, off/low/medium/high): the stroke follows a point that closes a fraction of the gap to the pointer each move (`_bdPenStep`), and on lift the tail steps up to where the pen lifted so the end is not clipped. Harness: 14 new checks, 151 to 165 (retargeted dblclick, caption edit, touch double-tap, alignment set/no-op/re-render/allowlist, stabilization step and strip).

## Finish plan (next session)

Jordan's call on 2026-09-04: wrap the project up in one more session. "Done" for the personal-tool scope means the four things below; Phase 5 (the consumer gate) stays triggered work and is out of scope unless Jordan says otherwise.

**Before the session (Jordan):** run `migrations/021_script_media.sql` in the Supabase SQL editor (idempotent; safe before the client ships). It adds the `script-media` bucket, `script_scenes.thumbnail_path`, the storage policies, and the updated shared-link RPCs. It is the only step in the plan that needs a hand other than mine.

**Definition of done, in order of work:**

1. **Phase 2d, every write goes through a store.** Move the remaining writes off the views and onto their stores: calendar events (`calendar.add/remove`), outreach targets and lists, profile and settings (profile, platforms, rate card, contract defaults, audience, invoicing settings; the `profile` store owns `sbUpdateProfile` and the rate-card writes), media-kit stats. Mechanical proof: a new static guard in `tests/checks.mjs` fails on any `db.sb(Add|Update|Delete|Create|Save)` call inside `src/views/` (Boards and Scripts editors excepted until step 3 touches them, then they go too). Then `mount(el)`: each view's render takes the container it paints into, so views stop reaching for `document.getElementById('view-x')`; the router passes it. Ship, push, live probe.
2. **Phase 3 leftovers.** (a) Script thumbnails to Storage: client uploads the compressed JPEG to `script-media/{owner}/{script}/{uuid}.jpg`, saves `thumbnail_path`, renders through a signed URL (the boards `_bdResolveSignedUrls` pattern), shared editors go through `patch_shared_scene(p_thumbnail_path)`; legacy `thumbnail_data` rows keep rendering. (b) Pointer-based scene reorder replacing the HTML5 drag-and-drop in `_bindDragDrop`, so it works on touch. (c) Board rollback on a failed item save (retry exists; on final failure restore the pre-save item and say so). Ship, push, live probe.
3. **Phase 4 remainder, the design system.** Tokens first (`--space-1..8`, `--radius-sm|md|lg`, `--shadow-soft`, `--overlay`, `--accent-dim`, `--teal-dim`, the type scale with `clamp()` on display), then components in the audit's order: `.empty`, `.toast`, `.btn` with variants (fold the 29 button classes), `.popover`, `.card` (`--kpi`, `--clickable`), `.field`, `.sheet` (bottom sheet under 640px, centered dialog above, body-mounted, `role="dialog"`, Escape, focus trap, focus return), `.row` (Tasks, Invoices, Outreach, Scripts rows). Verification: add `pixelmatch` + `pngjs` as dev dependencies and a `tests/diff.mjs` that compares the smoke screenshots against a committed baseline, with a per-view tolerance; refactors that must not change pixels (tokens, class folding) are checked at zero drift, the intentional changes (sheet a11y, contrast) are eyeballed from the screenshots and the baseline is re-committed. If time runs short, stop after tokens plus the first components and record where; this is the one step that can be cut without leaving the app inconsistent, as long as each component fold ships whole.
4. **Close-out.** README pass (drop the stale sections: legacy root files, the old migration table, "Self-Hosting Plan", "Key Areas for Future Work"; describe the module, store and router layout as it is), delete `V1-AUDIT.md` (superseded) and propose the dead root files for deletion (`api_server.py`, `command-center.js`, `google-apps-script.js`, the expense JSON and spec, `test-console.html`), and a final progress-log entry that marks the audit closed for the personal-tool scope, with Phase 5 listed as the trigger list for a consumer moment.

**Rules for the session:** harness green before every commit (`npm run lint && npm test`), one commit per numbered step at minimum, push after each (push deploys, and the live probe follows every push), phone check by Jordan at the end. Nothing in the plan needs a migration beyond 021.

**After this session, the standing triggers (not backlog):** Phase 5 consumer gate on a real consumer moment; per-user cost model when pricing is needed; `introspect.sql` any time schema drift is suspected.
