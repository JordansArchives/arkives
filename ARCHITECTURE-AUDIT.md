...
2. **Phase 2b (modules):** Vite or native ES modules, `lib/` extraction, per-domain stores, router with `mount` and `unmount`. Two facts the next session must plan around before touching a file. First, `tests/checks.mjs` and `tests/smoke.mjs` reach the app through its classic-script globals (bare assignments like `TASKS = [...]`, overrides like `window.sbUpdateTask = ...`, direct calls like `navigate(v)`); real ES modules make every one of those invisible, so the harness needs a deliberate test seam (an exported `__test` object, or modules that publish their state and functions onto one namespace) designed and shipped before the conversion, or the safety net disappears exactly when it is needed. Second, Vite adds a build step, and Cloudflare Workers Builds only runs the build command set in the dashboard: pushing a Vite layout before that field is set deploys an empty `dist/`. Native ES modules with no bundler (`<script type="module">`, imports resolved by the browser, `modulepreload` hints, CDN libraries kept as the pinned UMD globals they already are) avoid both the build step and the deploy coordination, at the cost of minification and hashed filenames the app has never had; given README's "zero build overhead" preference, decide this explicitly at the start of that session.
