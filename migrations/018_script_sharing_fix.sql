...
-- shared session.
--
-- Also writes down schema drift: scripts.share_mode exists on
-- live but appears in no migration ('none'|'view'|'edit', app
-- code has used it all along). Made official here.
--
...
CREATE OR REPLACE FUNCTION public.get_shared_script(p_token UUID)
...
AS $$
  ...
  FROM scripts s
  WHERE s.share_token = p_token AND s.share_mode IN ('view','edit')
$$;

CREATE OR REPLACE FUNCTION public.get_shared_script_scenes(p_token UUID)
...
CREATE OR REPLACE FUNCTION public._shared_script_id(p_token UUID)
RETURNS UUID
...
BEGIN
  ...
  WHERE s.share_token = p_token AND s.share_mode = 'edit';
...
CREATE OR REPLACE FUNCTION public.update_shared_script_title(p_token UUID, p_title TEXT)
RETURNS VOID
...
CREATE OR REPLACE FUNCTION public.add_shared_scene(p_token UUID, p_sort_order INTEGER)
RETURNS TABLE (id UUID, script_id UUID, sort_order INTEGER,
               script_text TEXT, scene_description TEXT, thumbnail_data TEXT)
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = public
...
CREATE OR REPLACE FUNCTION public.delete_shared_scene(p_token UUID, p_scene_id UUID)
RETURNS VOID
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = public
AS $$
...
CREATE OR REPLACE FUNCTION public.reorder_shared_scenes(p_token UUID, p_scene_ids UUID[])
RETURNS VOID
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = public
AS $$
...
REVOKE ALL ON FUNCTION public.get_shared_script(UUID) FROM public;
REVOKE ALL ON FUNCTION public.get_shared_script_scenes(UUID) FROM public;
REVOKE ALL ON FUNCTION public._shared_script_id(UUID) FROM public;
REVOKE ALL ON FUNCTION public.update_shared_script_title(UUID, TEXT) FROM public;
REVOKE ALL ON FUNCTION public.patch_shared_scene(UUID, UUID, TEXT, TEXT, TEXT) FROM public;
REVOKE ALL ON FUNCTION public.add_shared_scene(UUID, INTEGER) FROM public;
REVOKE ALL ON FUNCTION public.delete_shared_scene(UUID, UUID) FROM public;
REVOKE ALL ON FUNCTION public.reorder_shared_scenes(UUID, UUID[]) FROM public;

GRANT EXECUTE ON FUNCTION public.get_shared_script(UUID) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_shared_script_scenes(UUID) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.update_shared_script_title(UUID, TEXT) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.patch_shared_scene(UUID, UUID, TEXT, TEXT, TEXT) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.add_shared_scene(UUID, INTEGER) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.delete_shared_scene(UUID, UUID) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.reorder_shared_scenes(UUID, UUID[]) TO anon, authenticated;
